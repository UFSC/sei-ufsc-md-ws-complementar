<?
/**
* CONTROLADORIA GERAL DA UNIпїЅO
*
...
class SeiComplementarRN extends InfraRN {
  ...
  protected function listarDocumentoControlado(WSEntradaListarDocumentoDTO $objWSEntradaListarDocumentoDTO){
    try{
    	...
    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarAndamentoGeracao())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para andamento de geraзгo invбlido.');
    	}
    	 
    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarAssinaturas())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para assinaturas invбlido.');
    	}

    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarPublicacao())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para publicaзгo invбlido.');
    	}

        if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarDestinatarios())) {
            $objInfraException->adicionarValidacao('Sinalizador de retorno para destinatбrios invбlido.');
        }
        ...
        if ($objDocumentoDTO!='') {
            ...
            if ($objDocumentoDTO->getStrProtocoloDocumentoFormatado()==''
                ...
                $objInfraException->lancarValidacao('пїЅ necessпїЅrio informar ao menos um dos parпїЅmetros da pesquisa.');
    	...
    	if ($arrDto==null){
    		$objInfraException->lancarValidacao('Nenhum documento encontrado com os parпїЅmetros informados.');
      }
      ...
      foreach($arrDto as $objDocumentoDTOEncontrado) {
           ...
           * if ($objUnidadeDTO != null) {
              ...
              if (count($objProtocoloRN->pesquisarRN0967($objPesquisaProtocoloDTO)) == 0) {
                  $objInfraException->lancarValidacao('Unidade [' . $objUnidadeDTO->getStrSigla() . '] nпїЅo possui acesso ao documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '].');
              }
          }*/
          ...
          if ($objOperacaoServicoRN->contar($objOperacaoServicoDTO) == 0) {
              if ($objUnidadeDTO == null) {
                  $objInfraException->lancarValidacao('Nenhum serviзo configurado para consulta deste documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '] pelo Serviзo [' . $objServicoDTO->getStrIdentificacao() . '].');
              } else {
                  $objInfraException->lancarValidacao('Nenhum serviзo configurado para consulta deste documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '] na unidade [' . $objUnidadeDTO->getStrSigla() . '] pelo Serviзo [' . $objServicoDTO->getStrIdentificacao() . '].');
              }
          ...
          //verifica se o usuпїЅrio jпїЅ tem acesso ao processo
      ...
      throw new InfraException('Erro no serviзo de consulta de documento.',$e);
    }
  }
    ...
    protected function listarProcedimentoControlado(WSEntradaListarProcedimentoDTO $objWSEntradaListarProcedimentoDTO){
        ...
        try{
            ...
            if ($arrDto==null){
                $objInfraException->lancarValidacao('Nenhum processo encontrado para os parпїЅmetros informados.');
            }
            ...
            foreach($arrDto as $objProcedimentoDTO) {
                ...
                if ($objUnidadeDTO != null) {
                    ...
                    $objProtocoloRN = new ProtocoloRN();
                    if (count($objProtocoloRN->pesquisarRN0967($objPesquisaProtocoloDTO)) == 0) {
                        $objInfraException->lancarValidacao('Unidade [' . $objUnidadeDTO->getStrSigla() . '] nпїЅo possui acesso ao processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '].');
                    }
                }
                ...
                if ($objOperacaoServicoRN->contar($objOperacaoServicoDTO) == 0) {
                    if ($objUnidadeDTO == null) {
                        $objInfraException->lancarValidacao('Nenhum serviзo configurado para consulta deste processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '] pelo Serviзo [' . $objServicoDTO->getStrIdentificacao() . '].');
                    } else {
                        $objInfraException->lancarValidacao('Nenhum serviзo configurado para consulta deste processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '] na unidade [' . $objUnidadeDTO->getStrSigla() . '] pelo Serviзo [' . $objServicoDTO->getStrIdentificacao() . '].');
                    }
                }
            ...
            throw new InfraException('Erro no serviзo de consulta de procedimento.',$e);
        }
    }
    ...
    protected function listarAndamentosControlado(WSEntradaListarAndamentosDTO $objWSEntradaListarAndamentosDTO){
        try{
            ...
            if ($dto==null){
                $objInfraException->lancarValidacao('Processo '.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().' nпїЅo encontrado.');
            }
            ...
            throw new InfraException('Erro no serviзo de consulta de andamentos.',$e);
        }
  }
  ...
  private function prepararParticipantes($Interessado){
      ...
      //como nпїЅo tem paginaпїЅпїЅo пїЅ bom limitar
    ...
    private function prepararClassificacaoAssunto($ClassificacaoAssunto){
        ...
        if (count($arrAssuntos)==0){
            throw new InfraException('Nenhuma ClassificaпїЅпїЅo de Assunto encontrado!'); }
  ...
  private function prepararUnidadeProcedimentoAberto($numIdUnidade, $numIdUsuario){
      ...
      if (count($arrObjPendenciaDTO)==0){
          throw new InfraException('Nenhum Procedimento com PendпїЅncia para a Unidade Informada!'); }
  ...
  private function adicionarCriteriosProcessoDocumento(OperacaoServicoDTO $objOperacaoServicoDTO, ProcedimentoDTO $objProcedimentoDTO, DocumentoDTO $objDocumentoDTO){
    ...
    //qualquer sпїЅrie em qualquer tipo de procedimento
    ...
    //esta sпїЅrie em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sпїЅrie neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sпїЅrie neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
  ...
  private function adicionarCriteriosUnidadeProcessoDocumento(OperacaoServicoDTO $objOperacaoServicoDTO, UnidadeDTO $objUnidadeDTO, ProcedimentoDTO $objProcedimentoDTO, DocumentoDTO $objDocumentoDTO){
    ...
    //qualquer sпїЅrie em qualquer unidade em qualquer tipo de procedimento
    ...
    //esta sпїЅrie em qualquer unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sпїЅrie nesta unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sпїЅrie em qualquer unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sпїЅrie nesta unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sпїЅrie em qualquer unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sпїЅrie nesta unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sпїЅrie nesta unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
